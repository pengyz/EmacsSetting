...
;; $URL: svn+ssh://rubyforge.org/var/svn/emacs-rails/trunk/inflections.el $
  ...
  (when (stringp str)
    (or (car (member str inflection-uncountables))
        ...
        (loop for (from to) in inflection-singulars
              for singular = (string-ext/string=~ from str (sub to))
  ...
  (when (stringp str)
    (or (car (member str inflection-uncountables))
        ...
        (loop for (from to) in inflection-plurals
              for plurals = (string-ext/string=~ from str (sub to))
